class WordProblem
  ...
  def answer
    split_array = split_operations.flatten

    raise ArgumentError if split_array.size < 3

    total = split_array.shift.to_i
    split_array.each_with_index do |element, index|
      case element
      when 'plus' then total += split_array[index + 1].to_i
      when 'minus' then total -= split_array[index + 1].to_i
      when 'divided' then total /= split_array[index + 1].to_i
      when 'multiplied' then total *= split_array[index + 1].to_i
      end
      split_array.delete_at(index + 1)
    end
    total
  end

  def split_operations
    string.scan(/(plus|minus|divided|multiplied|[0-9]+|[-0-9]+)/)
  end
end
