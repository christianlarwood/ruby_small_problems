...
class Phrase
  ...
  def word_count
    # cleaned_string = phrase.gsub(/[^a-z0-9\']/, ' ').split
    result = Hash.new(0)
    cleaned_string.uniq.each do |word|
      result[word] = cleaned_string.count(word)
...
# Test Cases:
test = Phrase.new("Joe can't tell between 'large' and large.")
p test.word_count
