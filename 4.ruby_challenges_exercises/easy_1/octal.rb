...
What's the expected input(s):
  - integer in decimal base 10 notation
What's the expected output(s):
  - integer in base 8 notation
What are the rules (explicit & implicit requirements)?:
  - last num is 8^0, num in -2 is 8^1, etc. 
  - sum up the nums
  - invalid octal = 0
...
Describe your mental model (interpretation) of the problem:
  - given an integer in base 10 form, find it's base 8 equivalent
...
Algorithm: What are the exact steps that will transform the input(s) to the desired output(s)? Use your mental model and data structure.
  - create array of letters to validate
  - return 0 if string contains letters
  - convert string to integer
  - split the integer into digits
  - iterate over each digit in reverse order and multiply by 8^index
  - sum the new integers
...
class Octal
  ...
  def to_decimal
    if valid_string
      array_of_digits = string.to_i.digits
      result = []
